  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        cell: "size-9 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-neutral-100/50 [&:has([aria-selected])]:bg-neutral-100 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 dark:[&:has([aria-selected].day-outside)]:bg-neutral-800/50 dark:[&:has([aria-selected])]:bg-neutral-800",
      ...
      components={{
        Dropdown: ({ value, onChange, options }: DropdownProps) => {
          ...
          const handleChange = (newValue: string) => {
            const optionValue = options?.find(opt => opt.value?.toString() === newValue)?.value;
            
            if (onChange && optionValue !== undefined) {
              const changeEvent = {
                target: { 
                  value: optionValue
                },
              ...
              console.warn('Could not find option for new value or onChange is missing:', newValue);
            }
          };
        ...
        Chevron: ({ orientation, ...props }) => {
          if (orientation === 'left') {
            return <ChevronLeft className={cn("size-4")} {...props as React.HTMLAttributes<SVGElement>} />
          }
          if (orientation === 'right') {
            return <ChevronRight className={cn("size-4")} {...props as React.HTMLAttributes<SVGElement>} />
          }
          return <></>;
        }
      }}
