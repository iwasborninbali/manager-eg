...
4. Компоненты разделов дашборда
	...
	•	Аналогичные компоненты, каждый отвечает за свою зону (бюджеты, зарплаты, счета).
...
5.1 Создание проекта
	...
	•	Пользователь вводит номер, название, бюджет, дату и выбирает менеджера (из роли MANAGER или CEO).
...
5.2 Просмотр/редактирование проекта
	...
	•	Модальное окно с деталями проекта (название, номер, заказчик, бюджет, менеджер, статус и т.д.).
	•	Подтягивает данные о назначенном менеджере, а также загруженные счета (через InvoicesSection) и итоги по бюджету.
	•	Для роли CEO доступно редактирование проекта:
	•	EditProjectDialog позволяет менять поля (имя, бюджет, менеджера, статус и др.), отправляет update в таблицу projects.
...
6. Работа со счетами (Invoices)
	...
	•	Вызывает onTotalAmountChange для пересчёта фактически использованного бюджета.
...
8. Итоговая логика взаимодействия
	...
	•	Суммы счетов влияют на остаток бюджета.
...
Управление бюджетом по проектам
...
Аналогично менеджеру, указывает категорию (включая «Бюджет отдела» с подпунктами EG, Brand Group, Imagineers), номер проекта, информацию о платеже, файлы (multiple), комментарии при необходимости
...
Дополнительное обсуждение подкатегорий при подаче счета "Бюджет отдела"
...
Реализация подкатегорий при выборе "Бюджет отдела"
Тестирование:
